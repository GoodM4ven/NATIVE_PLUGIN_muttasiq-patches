...
- Seeds Android's runtime sqlite database from Muttasiq's bundled Quran-reader snapshot before `app:native-bootstrap`, so the reader opens with a complete dataset instead of waiting on on-device Quran migrations.
...
- `native-bundle-extract`: prunes the dormant Quran exegesis payload plus stale `public/build/assets/*` entries not referenced by `public/build/manifest.json` from Android's generated `laravel_bundle.zip`, patches `LaravelEnvironment.kt` unzip behavior to use streaming extraction with ZIP slip protection, keeps a runtime exegesis skip fallback, seeds the runtime sqlite file from `database/native-quran-reader.sqlite` when present, and swaps raw `migrate --force` for `app:native-bootstrap --no-interaction`.
