...
- Adds native `SecureStorage` bridge implementations for Android and iOS so the app can persist device auth state without the paid NativePHP secure-storage plugin.
...
- Patches `WebViewManager.kt` to install early request capture for `Livewire` and `Filament`, while preserving NativePHP 3.2 request-id forwarding (`X-NativePHP-Req-Id`) used by Android POST body replay. It also keeps custom app deep links on the Android side from being rendered as a WebView 404, so the app's configured custom scheme can round-trip cleanly.
...
- Patches `PHPBridge.kt` to make the embedded PHP runtime process-wide: the single-thread `phpExecutor` and the `runtimeInitialized` flag are promoted from per-instance fields to `companion object` statics (`sharedPhpExecutor` / `sharedRuntimeInitialized`, guarded by a class-level monitor). Upstream gives every `PHPBridge` instance its own executor + init flag, but `nativeRuntimeInit()` primes process-global PHP/TSRM state bound to whichever OS thread first ran it. When a second instance is created (activity recreate/resume) it re-inits PHP on a different pool thread and rebinds that global state, so the first instance's still-live request thread then `SIGSEGV`s in `ts_resource_ex+276` (via `OnUpdateBool` during `php_tsrm_startup_ex`) on its next `php_embed_init`. Funneling all PHP work through one shared thread and priming exactly once per process keeps global PHP/TSRM state touched only from its owning thread. Observed as a native crash when a websocket (`dataSynced`/`dataOverridden`) realtime event triggered a `/native-sync/pull` request right after an instance churn.
...
- Makes Android QPC page-font resolution self-healing: `resolveBundledQpcFontFile` now also recognizes the static `vendor/arabicable/qpc-v2/p{page}.{ttf,woff2}` publish path (not just the `qpc-v2-fonts/{page}.{ext}` route), resolving it directly from the immutable bundled raw-data tree and opportunistically re-publishing it whenever the on-device published copy is missing or stale. Fixes specific Quran pages rendering with the wrong/garbled glyphs on Android (observed on pages 445-447) after their published static font copy went missing while `isBinaryAssetPath()` was blocking the PHP fallback that would otherwise have re-published it.
...
- Guards Android `runBaseArtisanCommands` against a PHP ZTS TSRM re-entrant `SIGSEGV` crash (`ts_resource_ex+276` via `OnUpdateBool` during `php_tsrm_startup_ex`) that occurs when `native_run_artisan_command` (JNI) is the first PHP call: calls `phpBridge.ensureRuntimeInitialized()` before the first artisan command so `nativeRuntimeInit()` primes the global PHP/TSRM state before `php_embed_init` triggers `php_tsrm_startup_ex`. Mirrors the initialization guard already present in `bootPersistentRuntime` and classic `handleLaravelRequest`.
...
- `native-secure-storage`: registers `SecureStorage.Set`, `SecureStorage.Get`, and `SecureStorage.Delete` bridge functions on Android and iOS using encrypted local storage.
...
- `native-android-assets`: also resolves the static `vendor/arabicable/qpc-v2/p{page}.{ext}` publish path from the bundled raw-data tree and re-publishes it on demand if the on-device copy is missing, so a desynced/evicted publish cache cannot leave a Quran page stuck rendering the wrong page font.
...
- `native-bundle-extract`: guards `runBaseArtisanCommands` with `phpBridge.ensureRuntimeInitialized()` to prevent a TSRM re-entrant SIGSEGV crash on app-update cold starts where PHP is first invoked through `native_run_artisan_command` without a prior `nativeRuntimeInit()` call.
